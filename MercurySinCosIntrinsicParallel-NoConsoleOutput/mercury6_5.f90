!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!
!      MERCURY6_1.FOR    (ErikSoft   3 March 2005)
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!
...
! Local
      ...
      real*8 hx,hy,hz,h2,p,rr0,rr1,rv0,rv1,temp,e,v2,rr_min
...
!      end if
!
! Distance of closest massive body to central body
      rr_min = 9.9d99
      do j = i0, nbig
        if (algor.eq.11) then
...
!
! If outside other massive bodies, is an artifact due to barycentric motion
          if (rr0.gt.rr_min) goto 100
!
          if (algor.eq.11) then
...
! Local
      integer i,j
      real*8 dx,dy,dz,s,s_1,s2,s_3,rc,rc2,q,q2,q3,q4,q5,tmp2,faci,facj,sqrtS2
!
!------------------------------------------------------------------------------
!
      if (i0.le.0) i0 = 2
      !
!$omp parallel shared(m,x,a,rcrit,i0,nbod,nbig,stat) private(dx,dy,dz,s,s_1,s2,s_3,rc,rc2,q,q2,q3,q4,q5,tmp2,faci,facj,sqrtS2,j)
!$omp do
      do i = i0, nbig
            do j = i0, nbod
                  if (j.ne.i) then
...
! et al. (1973) Astron. J. 78, 211-225, with magnitude determined by the
...
! Read data style
 120    read (11,'(a150)') string
        ...
        if (c3.eq.'Car'.or.c3.eq.'car'.or.c3.eq.'CAR') then
          ...
          informat = 3
        else if (c3.eq.'Bas'.or.c3.eq.'bas'.or.c3.eq.'BAS') then
          informat = 4
        else
...
!
! If using barycentric orbit for small body, it must have the same epoch
! as the big bodies
        if (informat.eq.4.and.epoch(nbod).ne.time) then
          write (*,'(/,2a)') ' ERROR: Programme terminated. See', &
            ' information file for details.'
          write (23,'(/,3a)') ' ERROR: Big and small bodies must', &
            ' all have the same epoch if reading in barycentric', &
            ' elements.'
          stop
        end if
!
! If required, read Cartesian coordinates, velocities and spins of the bodies
 ...
 135    read (11,'(a150)',end=666) string
        ...
        if (informat.eq.1) then
          ...
          temp = m(nbod)  +  m(1)
!
! If using barycentric elements for small bodies:
          if (informat.eq.4.and.j.eq.2) temp = mbig  +  m(nbod)
!
...
!
! If using barycentric elements for small bodies convert to heliocentric elements
...
!
! Update barycentric position and velocity of central body
        if (j.eq.1) then
...
!
 ...
 140    close (11)
!
! Calculate barycentric position and velocity of central body
        if (j.eq.1) then
          xcen(1) = xcen(1) / mbig
          xcen(2) = xcen(2) / mbig
          xcen(3) = xcen(3) / mbig
...
!
      ...
      subroutine drift_kepmd(dm,es,ec,xkep,s,c)
...
!...    calc initial guess for root
      fac1kep = 1.d0/(1.d0 - ec)
      qkep = fac1kep*dm
      fac2kep = es*es*fac1kep - ec/3.d0
      xkep = qkep*(1.d0 -0.5d0*fac1kep*qkep*(es -qkep*fac2kep))

!...  excellent approx. to sin and cos of x for small x.
! FGT, Aug. 30th, 2017, Using the following comment...
!            s,c        ==> sin and cosine of x (real*8 scalars)
! Replace the following 3 statements by their corresponding "alternatives": sin & cos
!        y = x*x
...
!...    Compute better value for the root using quartic Newton method
      fkep = xkep - ec*s + es*(1.-c) - dm
      fpkep = 1. - ec*c + es*s
      fppkep = ec*s + es*c
      fpppkep = ec*c - es*s
      dxkep = -fkep/fpkep
      dxkep = -fkep/(fpkep + 0.5*dxkep*fppkep)
      dxkep = -fkep/(fpkep + 0.5*dxkep*fppkep + 0.16666666666666666*dxkep*dxkep*fpppkep)
      xkep = xkep + dxkep
