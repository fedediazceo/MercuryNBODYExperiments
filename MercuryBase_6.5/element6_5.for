c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      ELEMENT6.FOR    (ErikSoft   21 September 2009)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
...
c
      ...
      include 'mercury.inc'
c
      integer itmp,i,j,jj,k,l,iback(NMAX),precision,lenin
...
c What type elements does the user want?
  ...
  45  read (10,'(a250)') string
      ...
      if (c2.eq.'ce'.or.c2.eq.'CE'.or.c2.eq.'Ce') then
        ...
        centre = 2
c      else if (c2.eq.'cb'.or.c2.eq.'CB'.or.c2.eq.'CL'.or.c2.eq.'cl'
c     %  .or.c2.eq.'Cl') then
c        centre = 3
      else
...
c Loop over each time slice
 100    continue
        line_num = line_num + 1
        read (10,'(3a1)',end=900,err=667) check,style,type
        line_num = line_num - 1
        backspace 10
c
...
c
        if (type.eq.'a') then
          line_num = line_num + 1
          read (10,'(3x,i2,a62,i1)',end=900,err=666) algor,cc(1:62),
     %      precision
c
...
c Check for data corruption
            do itmp = 1, 51
              if (ichar(c(j)(itmp:itmp)).eq.12) goto 666
            end do
          end do
c
...
c
          ...
          line_num = line_num + 1
          read (10,'(3x,a14)',err=667) cc(1:14)
c
c Decompress the time and number of objects
          time = mio_c2fl (cc(1:8))
          old_nbig = nbig
          old_nsml = nsml
          nbig = int(.5d0 + mio_c2re(cc(9:16),  0.d0, 11239424.d0, 3))
          nsml = int(.5d0 + mio_c2re(cc(12:19), 0.d0, 11239424.d0, 3))
c If some lines are missing...
          if (nbig.ne.old_nbig.or.nsml.ne.old_nsml) goto 666
          nbod = nbig + nsml
...
c Read in strings containing compressed data for each object
          do j = 1, nbig + nsml
            line_num = line_num + 1
            read (10,fin,err=667) c(j)(1:lenin)
c Check for data corruption
            do itmp = 1, lenin
              if (ichar(c(j)(itmp:itmp)).eq.12) goto 667
            end do
c
c For each object, decompress its orbital variables
            code(j) = int(.5d0 + mio_c2re(c(j)(1:8), 0.d0,
     %        11239424.d0, 3))
c
c Check if file is corrupted
            if (code(j).lt.1.or.code(j).gt.nbod) goto 667
            do jj = 1, j - 1
              if (code(j).eq.code(jj)) goto 667
            end do
c
            l = j + 1
            m(l) = el(18,code(j)) * K2
...
c Convert to Keplerian orbital elements
            gm = (mcen + el(18,k)) * K2
            if (algor.eq.11.and.j.gt.1) gm = gm + m(2)
...
c Move on to the next time slice
        goto 100
c
c If input file is corrupted, go to next uncorrupted special output
 666    continue
...
c
        ...
        if (timestyle.eq.0.or.timestyle.eq.2) then
          write (*,'(a,f20.5,a)') '       Time: ',t1,' days'
        else if (timestyle.eq.1) then
          write (*,'(a,i12,1x,i2,1x,f7.5,a)') '       Time: ',year,month,t1
        else
          write (*,'(a,f20.7,a)') '       Time: ',t1,' years'
        end if
c
        write (*,*) '       Searching for next special output'
        check = ' '
        type = ' '
        do while (ichar(check).ne.12.or.type.ne.'a')
          line_num = line_num + 1
          read (10,'(3a1)',end=900) check,style,type
        end do
        line_num = line_num - 1
        backspace 10
        goto 100
...
c
        write (*,*) '       Searching for next normal output'
        c1 = ' '
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MCO_H2CB.FOR    (ErikSoft   29 August 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c coordinates (for use in element6.for only).
c
c------------------------------------------------------------------------------
c
